...
//   .git/HEAD symbolic ........... 23 bytes   ref: refs/heads/master\n
...
(function () {
  ...
  var SWITCH_MASTER = { act: "switch", ref: "refs/heads/master" };
  var DETACH = { act: "detach", commitId: "87ab8c9" };

  var SETUP = [WRITE, STORE, SAVE, NAME];
  var BRANCH_FEATURE = [{ act: "name", ref: "refs/heads/feature", commitId: "87ab8c9" }];
    ...
    steps: [
      {
        ...
        objects: { lens: "chain", acts: SETUP, fresh: 1, note: "one commit, one ref" }
      },
      {
        narr: "In its usual state, `HEAD` holds the **path to a ref file**. Opened as text, it reads `ref: refs/heads/master` followed by one newline, 23 bytes total. The file does not repeat any part of the commit - no id, no message, no tree. It names the ref, and the ref names the commit.",
        objects: { lens: "chain", acts: SETUP, fresh: 0, openHead: true, note: "HEAD points at a ref" }
      },
      {
        ...
        objects: { lens: "chain", acts: SETUP.concat(BRANCH_FEATURE, SWITCH_FEATURE), fresh: 0, openHead: true, note: "switched to feature" }
      },
      {
        narr: "Switching back rewrites `HEAD` again - this time to point at `master`. The refs have not moved; the only thing that changed is which one `HEAD` names. That indirection is what lets git update the right ref when a commit is saved.",
        objects: { lens: "chain", acts: SETUP.concat(BRANCH_FEATURE, SWITCH_FEATURE, SWITCH_MASTER), fresh: 0, openHead: true, note: "switched back to master" }
      },
      {
        narr: "The second state is called **detached** - `HEAD` holds a commit id directly, with no ref in between. In this mode, `HEAD` contains exactly what a ref file contains: one 40-character SHA-1 followed by a newline, 41 bytes. The symbolic pointer is gone; `HEAD` points straight at the commit, just as `master` does.",
        objects: { lens: "chain", acts: SETUP.concat(DETACH), fresh: 0, openHead: true, openHeadRaw: true, note: "detached HEAD, 41 bytes" }
      },
      {
        ...
        objects: { lens: "chain", acts: SETUP.concat(DETACH), fresh: 0, openHead: true, note: "one name, two modes" }
      },
      {
