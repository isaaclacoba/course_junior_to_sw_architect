...
(function () {
  ...
  window.LESSON_CONFIG = {
    ...
    steps: [
      {
        narr: "A commit named `87ab8c9` contains one tree, one parent field, one message - a fixed set of bytes. Hashing those bytes produces that id, which means the commit's **name comes from its contents**. Editing the message would produce different bytes, and different bytes hash to a different id. A commit cannot be edited - its name prevents it.",
      ...
      {
        narr: "The old commit has become **unreachable** - no ref names it, and no reachable commit names it as a parent. It is greyed out because nothing leads to it, but it is still there. The id `87ab8c9` still resolves to a commit object, and the tree and blob it pointed at are also untouched. Amending did not delete anything; it wrote one new commit and updated one ref.",
        objects: { lens: "chain", acts: SETUP.concat(AMEND), fresh: 0, note: "original commit orphaned" }
      },
      {
      ...
      },
      {
        ...
        objects: { lens: "chain", acts: SETUP.concat(AMEND), fresh: 0, note: "gc removes what nothing reaches" }
      }
    ]
  };
})();
