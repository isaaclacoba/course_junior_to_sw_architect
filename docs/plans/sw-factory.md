...
27. [x] **Complete backfill of the firehose** - verify: all 693 transcripts / 114,481 lines ingested to 81,022 rows in 3m00s at 399MB peak; watermark total equals the source line count exactly; a full re-run adds 0 rows; rung 1 now reads `observed` for 15 of 18 features
28. [x] **A coverage floor, so `absent` is earned** - verify: the 3 features that first read `absent` were each named in ONE session by a config file listing the slug; every feature with real work spans >=2 sessions; below the floor the machine goes `blind` and claims nothing - 3 sabotages fail by name
29. [ ] Watch a real feature walk the chain end to end - verify: one line of work goes `recall` -> `verifying` with each handoff made by an agent, not by me

## Progress
- 2026-08-06 Backfill done: 693 sessions, 81,022 rows. Streaming the ETL was not optional - one transcript is 153MB, and reading it whole held the file, the line array and the rows at once. Streamed with a byte-budgeted flush it peaks at 399MB and resumes at line granularity.
- 2026-08-06 `docs/journal/**` is served on the PUBLIC Pages site - verified `HTTP 200` + `PAR1` magic bytes on a committed decisions parquet. `activity/` was already gitignored (my earlier "not ignored" was a bad `check-ignore` call without the trailing slash), so the firehose never leaves this machine. It holds 164 sessions that touch client work under `/home/isaac/repos`, so that matters. The committed journal itself is clean: 0 client references in all 159 rows.
- 2026-08-06 Session-store enrichment is a silent no-op: the store holds 11 sessions and `repository` is null, so it adds nothing and never warns. Left alone; the transcripts carry the signal.
