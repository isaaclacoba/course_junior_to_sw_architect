...
3. [ ] Real hashing in the mockup - verify: typed content gives the id
   `git hash-object` gives, checked against real git for 5 inputs
4. [ ] Real object store - blob, tree, commit built from typed content - verify:
   every id matches real git's for the same content
5. [ ] All three lenses rendering from one `StoreState` - verify: the same state
   drawn as folder, chain and both, side by side
