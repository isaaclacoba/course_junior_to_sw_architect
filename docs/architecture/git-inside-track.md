...
- SHA-1 is implemented in plain JavaScript, not taken from `crypto.subtle`:
  `crypto.subtle` is absent outside a secure context (a LAN address during local
  development loses it) and it is async, which would put a promise inside every
  render. Proven against real git's ids in `_verify-store.mjs`.
...
- Tree entries sort as if every directory name ended in `/`, so `a.txt` comes
  before a directory `a`. The directory mode written into the object is `40000`,
  not the `040000` that `git cat-file -p` prints.
