...
| 4 - Evaluation (submodule) | 16-18 | 3 / 3 |
...
- **2026-07-29 (Phase 4 item 18: per-concept covered state)** - Read the shared `course_concept_progress`
  key (written by item 17) on the two concept surfaces, course-side only. `glossary.html`: a `Covered`
  pill on each concept the learner has answered correctly, plus a ` - N covered` tail on the count line
  (self-contained CSS `.g-covered` on the styles.css tokens). `page-shell.js`: `conceptProgress()` reads
  the key and `renderAgenda` adds `is-covered` (a leading checkmark + accent, `.agenda-chip.is-covered`
  in styles.css) to any chip whose concept is covered. Empty/absent progress is safe (no pill, no suffix,
  no class). VERIFIED in a real headless browser via an iframe harness that seeds the key then reads the
...
- [x] **18. Per-concept progress** on glossary + agenda from stored results. Verify: a
