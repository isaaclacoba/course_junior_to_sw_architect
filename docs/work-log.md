...
## 2026-07-30 16:02 CEST - visualizer: collapse redundant call-entry step; editor width as a token
- start: two follow-ups from the layout/console work. (a) A method call showed its first line twice in a row - the instant the frame is pushed, then again as the first statement runs (the tracer's Enter records the fresh frame at the first statement's line) - so on a one-line method you clicked Next and nothing moved. (b) The wide-screen editor/aside widths were hardcoded (43rem / 33rem), with no way for a page or theme to retune them.
- end (2026-07-30 16:02 CEST): (a) added a pure pre-pass `collapseCallEntries` in the trace adapter (`src/core/exec-tracer-model.ts`, render-only - no Tracer.cs, no republish): it drops a call's entry snapshot when the very next step is the same line in the same freshly pushed frame. The stack still visibly grows at the surviving step (its depth is one deeper than the caller), so the "a call pushes a frame" beat is kept - it just lands together with the first statement instead of a dead click earlier. Guardrails: the first step (Main's own entry, no caller before it) is never dropped so a trace still starts in Main, and two statements authored on one line in the same frame are not merged (no push between them). Also made the "Traced N steps" status count the rendered steps (less the terminal beat) so it stays in step with the stepper after collapsing (e.g. the interface/Cat/Dog program now reads "Traced 7 steps" with an 8-step stepper, down from a 10-step one). (b) promoted the wide-screen column widths to tokens on the `.cl-vl` root - `--vl-editor-max` (43rem) and `--vl-aside-min` (33rem) - referenced in the >=1240px grid, so a page or theme can widen the editor with one declaration; the 1240px breakpoint stays literal (media conditions can't read custom properties) with a comment keeping it in step. Added two adapter tests (one-line method collapses; Main entry kept + same-line pair not merged) - typecheck clean, 128 tests, build, re-vendor js+css. Headless e2e on the Cat/Dog program: rendered steps dropped 10->8 (the two Speak entries collapsed), the console still shows Meow/Woof with the fresh line highlighted, status "Traced 7 steps" matches the /8 stepper, default layout pixel-unchanged; and an override `.cl-vl { --vl-editor-max: 62rem }` measurably widened the editor (688px -> 809px). Local only, no push.

## 2026-07-30 16:19 CEST - visualizer: effect-based step captions (no AI)
- start: the step caption just echoed the highlighted line ("Running this line: `Console.WriteLine(...)`"), so it was redundant with the editor and the terminal "The program has finished." beat felt empty. Making it explain each line seemed to need AI "smart comments", which we're not doing yet.
- end (2026-07-30 16:19 CEST): replaced the echo with a deterministic effect-based caption derived from the same per-step diff that already drives the highlights - no AI. New `describeStep` in the trace adapter (`src/core/exec-tracer-model.ts`, render-only - no Tracer.cs, no republish) picks the single most salient event: a call pushed ("Called `Speak()` on `Cat #1`", "Entered `Main`", "Called the `Clock` constructor"), a call returned ("`Speak()` returned to `Main`"), a value printed ("Printed `Meow`"), an object created ("Set `c` to a new `Cat`" / "Created a `Cat`"), a local changed ("Set `b` to `2`" / "Pointed `animal` at `Dog #1`"), or a static changed; it falls back to the source line only when nothing structural is detectable. When a return value is printed on the same line (`Console.WriteLine(c.Speak())`) the pop and the print land on one snapshot - the pop headlines the caption while the console panel below shows the printed line highlighted, so nothing is lost. The terminal beat now states the outcome concretely ("The program finished. It printed 2 lines." / "...without printing anything."). Added 5 adapter tests (each caption type, a direct print, the fallback, the concrete terminal) and updated the two that asserted the old strings - typecheck clean, 132 tests, build, re-vendor js. Headless e2e on the interface/Cat/Dog program now reads: Entered Main / Set animal to a new Cat / Called Speak() on Cat #1 / Speak() returned to Main / Set other to a new Dog / Called Speak() on Dog #1 / Speak() returned to Main / The program finished. It printed 2 lines. - backtick spans render as inline code chips, layout unchanged. Local only, no push.

## 2026-07-30 17:07 CEST - visualizer: code review of the session's changes + one fix
- start: review this session's visualizer diff (submodule `bd963c2..43e7d74`: console panel, collapse + editor width tokens, effect-based captions) for real bugs (off-by-one, undefined/empty captions, XSS/escaping of the caption `<code>` chips and console text, console incremental-diff correctness, step numbering after collapse) and fix any findings before it settles.
- end (2026-07-30 17:07 CEST): two independent reviews (a code-review sub-agent plus a direct read) agreed the changes are sound - escaping is safe end-to-end (`renderNarration`/`inline` run `escapeHtml` before the markdown transforms, and `ConsoleView` escapes with its own `esc()`), `collapseCallEntries` never drops the first/last step, `describeStep` guards every index access and always returns a non-empty string, and the "Traced N / STEP N/M" counts stay consistent after the collapse. One real (minor, cosmetic) finding: `inline()` applied `*italic*`/`**bold**` over the whole string including text already inside a `` `code` `` chip, so a printed value or string with asterisks (e.g. `Printed \`2 * 3 * 4\``) rendered a stray `<em>` inside the chip. Fixed at the single point (`src/core/narration.ts`): resolve code spans first, then apply emphasis only to the segments outside them - the markdown rule that a code span is literal. This also fixes the pre-existing source-line fallback path. Added two narration tests (asterisks in a code span stay literal; emphasis outside still formats when the code holds asterisks) - typecheck clean, 134 tests, build, re-vendor js. No compiler-host change, so no republish. Headless e2e on a program printing `2 * 3 * 4 = 24`: the console shows the asterisks literally, no `<em>`/`<strong>` leaks into any `<code>` chip, trace/step/console all work; the only page error is the unrelated `favicon.ico` 404. Local only, no push.

## 2026-07-30 21:17 CEST - reconcile onto remote master + publish the visualizer work
- start: the session's committed visualizer work (course branch `agents/effective-elk`, code-lab branch `save/effective-elk-viz`) needed to land on the shared remote. Chose to rebase onto the pushed `origin/master` of each repo (not the newer local, unpushed master that holds the other agent's large restructure) so our history stays simple and that agent absorbs the reconciliation on their next rebase.
- end (2026-07-30 21:17 CEST): both rebases were clean no-ops - our branches already sat directly on top of both remote masters (course 29 ahead / 0 behind `ba8c6e1`; code-lab 21 ahead / 0 behind `ed6848b`), so nothing replayed and no restructure conflicts arose. Moved the code-lab submodule off its fragile detached HEAD onto branch `save/effective-elk-viz` first. Pushed submodule-first so the superproject gitlink resolves on the remote: code-lab fast-forwarded `ed6848b..4ab31a6` -> origin/master, then the course fast-forwarded to origin/master (pushing `agents/effective-elk`). The course push triggers the GitHub Pages deploy workflow. Safety branches kept (`backup/effective-elk-pre-rebase-20260730-211118`, `backup/codelab-viz-pre-rebase-20260730-211118`, `save/effective-elk-viz`); rerere left enabled in both repos.

## 2026-07-30 21:27 CEST - fix commit attribution on code-lab and re-point the submodule
- start: four earlier code-lab commits (the AI-track scenes + SOLID refactor, the "going deeper" scenes, retrieval/planboard, and the tidy-up) were authored and committed as `Isaac Lacoba <isaac.lacoba@volvocars.com>`, which GitHub attributes to the denied `VCC-ILACOBA` account instead of `isaaclacoba`; every later commit already used `isaac.lacoba@gmail.com`. The user asked to fix the author on those four and force-push code-lab master.
- end (2026-07-30 21:27 CEST): rewrote the code-lab branch with `git filter-branch --env-filter`, remapping author+committer email `isaac.lacoba@volvocars.com` -> `isaac.lacoba@gmail.com` (name unchanged, dates/messages/Co-authored-by trailers preserved); the 11 pre-existing `isaac@local` commits and all tree content were left byte-identical (empty diff vs the pre-rewrite backup). SHAs cascaded, so code-lab HEAD `4ab31a6` -> `35af042`; force-pushed (lease-guarded on `4ab31a6`) to code-lab `origin/master` over HTTPS. Because the course pinned the submodule at the now-orphaned `4ab31a6`, re-pointed the gitlink to `35af042` and committed + pushed the course (fast-forward, over HTTPS) so `git submodule update` stays intact. Backups kept: code-lab `backup/codelab-pre-authorfix-20260730-212500` @ `4ab31a6`.

## 2026-07-31 08:44 CEST - close the build-lesson grading holes (verify probes + requireSource)
- start: the cycle-1 audit flagged five build lessons whose grading could be passed by a hardcoded constant (no hidden `verify` probe). Two of the five (`first-builds`, `wiring-it-up`) no longer exist after the course restructure, so scope is the four live build lessons: `test-doubles`, `writing-methods`, `lambdas`, `testing-basics`.
- end (2026-07-31 08:44 CEST): closed the holes with the right tool per lesson, since the engine's hidden-verify probe only fits some shapes. Key constraint: `build-engine.js` `buildProbe` keeps everything BEFORE `class Program` and replaces `class Program` onward with `verify.main`, so a probe can only re-run learner code that lives in a class declared before `Program`. (a) `test-doubles` fits perfectly - each task's learner writes a separate double (`FixedClock`/`StubFeed`/`SpyMailer`) before `Program`; added a `verify` probe per task that re-runs the double through its interface with a fresh assertion (Hour()==9; Total(5)==50 with a different qty; WasCalled starts false then Send() flips it). (b) `writing-methods`, `lambdas`, `testing-basics` keep their graded logic inside `Main`/`Program`, so a Main-replacing probe cannot reach it - hardened `requireSource` instead: guards target statement/operator forms (`+ age`, `return "minor"`, `if`/`?:`, `Category(16/18/40)`, `Category(age)`, the lambda call `addLeg(`/`isFourLegged(`/`enough(`/`reward(`, `== "Woof"`, `actual == expected`). Because `requireSource` runs on the whole source INCLUDING comments, reworded the few TODO comments that spelled the exact tokens (e.g. `Category(age)`, `addLeg(3)`, `isFourLegged (`) to intent-only - which also aligns with the AGENTS.md rule that a TODO states intent, not literal code. Validated with real dotnet 8 via a Node `vm` harness that reproduces `buildProbe`/`matches`/`requireSource` exactly: every solution passes all guards + compiles to `expected` + passes its probe, and a "sophisticated" cheat (has the technique tokens but hardcodes the printed answer) passes the visible case yet is blocked by >=1 guard on all 15 tasks. Content/data-only change (four lesson `*.js` files); no engine or host change, non-conflicting with the in-flight renderer work. Local only, no push.

## 2026-07-31 09:12 CEST - untaught-syntax pass (close cycle-1 cross-cutting item 1)
- start: the cycle-1 audit flagged six syntax tokens (`=>`, ternary `?:`, `$"..."`, `var`, `static`, nullable `string?`) used in lessons before any explanation. Re-derived the CURRENT 30-lesson practical reading order (the course grew a whole "Everyday essentials" Part since the audit) and traced each token's first appearance vs where it is taught, parsing every lesson config in a Node `vm` and scanning only the data strings (so JS arrow functions in the wrapper are not mistaken for C# `=>`).
- end (2026-07-31 09:12 CEST): two of the six are already handled by the grown course and need no change - `string?` is introduced at its first use in `foundations` task 5 ("a `?` type is allowed to be null"), and `$"..."` is introduced at its first use in `strings` task 0. The other four were closed with one-time notes at the earliest point each appears, "introduce once, early": (1) `static` - `foundations` task 1 now names `class Program`/`static void Main()` as the boilerplate frame, explained later; (2) `var` - `foundations` task 2 (Variables) now shows `var legs = 4;` equals `int legs = 4;` (its first real use is the `var x = new X()` harness in `control-flow`); (3) ternary `?:` - `control-flow` task 1 (the decisions lesson, #3) now introduces `condition ? a : b` as the compact either/or, before its first use in `linq` (#17); (4) `=>` expression-bodied - `reuse-without-regret` step 1 walk (#6, the first `=>` in the course, `Speak() => "Meow"`) now notes it is shorthand for a one-line body, before `lambdas` (#16) teaches the lambda `=>`. Prose-only edits to three files (`foundations.js`, `control-flow.js`, `reuse-without-regret.js`); validated with `node --check`, a config-parse check that the notes are in the rendered data, and a headless-Chrome DOM dump of all three pages (no `undefined`; the first-card notes render, the later-card notes confirmed in-data since build/walkthrough pages render one card at a time). Content/data-only, non-conflicting with the in-flight renderer work. Local only, no push.

## 2026-07-31 09:30 CEST - state the SOLID letter each Part 4/5 lesson embodies (audit item 3)
- start: the cycle-1 audit flagged that every Part 4 lesson (and the testing/refactor lessons) IS a SOLID letter but never names it, so the capstone's principle names land without a back-reference. Read each lesson's audit report to get the intended mapping rather than guessing.
- end (2026-07-31 09:30 CEST): added one explicit "In SOLID - ..." recap item to each of the five Part 4 lessons naming the letter it seeds, plus letter tags on the two testable-design habits that map. Mapping (honest, per the per-lesson audit reports): encapsulation -> S (one reason to change); interfaces -> O (a new type drops in without editing the caller) plus the abstraction D builds on; polymorphism -> L (any subtype stands in at one call site) reinforcing O; composition -> not a letter itself but keeps L honest (no is-a lie) and eases the O swap; dependency-injection -> D (receive an abstraction instead of building it); testable-design -> inject is D, one job is S. refactor-moves already maps each move to a principle, so it was left as-is. I (Interface Segregation) is intentionally still only in The SOLID Principles - adding it to the capstone is the separate item 7 (a real reshape, since the capstone's IReporter has one method and nothing to segregate). Verified: node --check on all six files, a config-parse check that every "In SOLID" item is present, headless-Chrome render of the recap cards confirming the bold letters (**S**/**O**/**L**/**D**) render via the shared renderInline, and a no-undefined DOM check on all six pages. Prose/data-only, non-conflicting with the in-flight renderer work. Recorded progress in the audit README (item 3 DONE; item 7 pending). Local only, no push.

## 2026-07-31 09:34 CEST - add the ISP milestone to the capstone (audit item 7)
- start: the cycle-1 audit's last SOLID gap - the flagship capstone walks S, DI/DIP, O, L but never Interface Segregation, because its `IReporter` has a single `Send` method and nothing to segregate. The user chose the "full ISP arc": introduce a real second capability so a fat-interface trap exists, then split it. Content change lives in `level3-exercise/` (the compiled Roslyn exercise, not the code-lab submodule), so CI rebuilds `level3-app` on deploy; no host edit needed since milestone count is data-driven (`Milestones.Count`) with no hardcoded 7.
- end (2026-07-31 09:34 CEST): inserted ISP as milestone 7 ("Keep each interface focused") and renumbered the substitutability (Liskov) milestone to 8 so L stays the climax. In `CapstoneContent.cs` the `ReferenceSolution` gained a focused `public interface ISummary { void Summarize(int passed, int failed); }`; `ConsoleReporter` now implements `IReporter, ISummary` (and `Main` calls `reporter.Summarize(1, 0)` so the capability is used, not dead code), while `SilentReporter` stays `IReporter`-only - that lean reporter is the segregation payoff. Added the milestone object (title, pass/todo, three tiered hints ending in an `ISummary` shape, a Why naming ISP, and a fat-vs-split mermaid diagram). In `StructuralChecks.cs` added `SegregateInterfacesRule` (MilestoneId 7) backed by a new `CapstoneSyntax.InterfacesAreSegregated()` that passes only when: the reporter interface has exactly one method (didn't absorb the summary), a DISTINCT one-method non-`Send` capability interface exists, some class implements that capability, AND some class implements the reporter interface but NOT the capability (proves the split bought something) - reusing the existing private `Implements`/`HasMethodNamed` statics. Renumbered `ProveSubstitutabilityRule` to MilestoneId 8 and registered the new rule between `SecondReporterRule` and it. Verified with a standalone Roslyn harness (dotnet 8, `Microsoft.CodeAnalysis.CSharp` 4.8.0) compiling the two exercise files + copied model records: `Run(ReferenceSolution)` = 8/8, `Run(StarterCode)` = 0/8, a fat-interface attempt (Summarize crammed onto IReporter) fails ONLY milestone 7, and a pre-ISP submission (single interface, two reporters) fails only 7 while 8 still passes independently; the reference program itself compiles and runs warning-free (`PASS` / `1 passed, 0 failed`). Updated the audit README (item 7 DONE + closing bullet) and `docs/audit/practical/capstone.md` (milestone table now 8 rows with the I tag, "seven"->"eight", ISP no longer listed as absent, verification status records the harness result). Scratch harness removed. Single course-repo commit; local only, no push.

## 2026-07-31 09:50 CEST - close the theme-split and difficulty-spike items (audit items 6 and 8)
- start: the last two cross-cutting cycle-1 items - item 6 (pick one running example family per Part) and item 8 (the "Practice the Basics drill 10" and "The SOLID Principles several-classes-at-once" difficulty spikes). Both were written against a pre-restructure course, so - as with items 1 and 3 - the first job was to re-derive the CURRENT state rather than execute the stale text. Ran two background explore agents in parallel (one for difficulty, one for theme), each briefed on the restructure, while reading refactor-moves.js myself; then verified every proposed change against the real files before touching anything.
- end (2026-07-31 09:50 CEST): both items were substantially resolved by the restructure; made one genuine bridge edit and left the rest honestly as-is. Findings: (item 8) practice-the-basics.js is now a 4-task build lesson that stacks one idea per task (old drill-10 synthesis gone), and the-solid-principles.js is now a 5-task build lesson, one principle per task, each writing one interface + 1-2 tiny classes (same grain as refactor-moves and Part 4) - so neither still jumps to several classes at once. The only residual was a continuity gap: refactor-moves' recap promises "the SOLID principles name these moves", but the SOLID hero intro never picked that thread up. Added one bridging sentence to the-solid-principles.html PAGE.hero.intro ("You already made these moves by hand in Refactor moves - SOLID just gives each one a name, and a feel for when it earns its keep."). Rejected the difficulty agent's proposed "LINQ 0" bridge task after checking linq.js: task 1 (Where) is already a single-operator intro that explicitly ties back to the Lambdas lesson, so no new task is warranted (and it would have bumped data-total). (item 6) the-solid-principles was rewritten from test-automation to the ANIMAL family, so Part 4 and both Part 6 build lessons now share animals; the remaining per-Part families are intentional and were left: Part 1's utility variety is deliberate syntax practice, Part 5's test doubles (FixedClock/SpyLog) are the right subject for a testing part, and the capstone's test-automation is the fixed compiled finale. Rejected the theme agent's proposed generics reskin: generics.js uses Box<T>/Pair<A,B> with int/string type args on purpose - the verify probes prove Box<int> and Box<string> both work, so a generic container is meant to be domain-neutral and re-skinning it to animals would undercut the lesson. Net change: one sentence in the-solid-principles.html (verified: renders in a headless-Chrome DOM dump, 0 "undefined"). Documented the reasoning in the audit README (items 6 and 8 marked DONE with the resolved-by-restructure + defensible-residuals rationale, closing-cycle-1 bullet updated) and the two per-lesson reports (level1-coding.md, level2.md) so a future session does not re-litigate. Content/data-only, non-conflicting with the in-flight renderer work. Local only, no push.

## 2026-07-31 10:21 CEST - name a few symbols before they are used (validation follow-up)
- start: while hand-validating the pages, a card was spotted (`control-flow` #2, boolean operators) that dove straight into `&&`/`||`/`!` without saying what they are. Scanned every card's `context` across all 28 lesson data files (parsed each config in a Node `vm`) for the same defect - a brand-new symbol/keyword used cold.
- end (2026-07-31 10:21 CEST): the course is mostly well-introduced; only four cards jumped into a new token without first naming it, all now fixed with one short opener each (short is better - just enough to orient). (1) `control-flow` #2 (Combine conditions): names `&&`/`||`/`!` as "C#'s three boolean operators: and / or / not" before the per-operator definitions. (2) `control-flow` #4 (foreach) - this is the first `foreach` in the whole reading order, yet opened "Inside a `foreach`..."; now "A `foreach` loop walks a sequence, handing you each item in turn" first. (3) `type-conversion` #3 (int.TryParse): glosses the `out` variable as "a second result the method hands back through an argument". (4) `strings` #0: interpolation was explained but never named - now "drop each value into an interpolated string, written `$"..."`". Deliberately left alone the tokens that already have a "what it is" opener (`??`/`?.`/`??=`/`int?`, `=>`, try/catch/throw, switch, while, for, generics `<T>`, virtual/override/abstract, using). Verified: `node --check` on all three files, a config-parse dump confirming the new prose is in-data, and headless-Chrome renders of the strings and boolean cards (the other two render one-card-at-a-time behind the build warm-up, confirmed in-data). Prose/data-only, non-conflicting with the in-flight renderer work.
