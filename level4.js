...
function setFeedback(text, isGood) {
  l4Feedback.innerHTML = renderInline(text);
...
function renderOptions(lesson) {
  ...
  shuffle(lesson.options).forEach((option, idx) => {
    ...
    btn.type = "button";
    btn.className = "btn";
    btn.innerHTML = renderInline(`${String.fromCharCode(65 + idx)}) ${option.text}`);
...
function renderLesson() {
  ...
  l4Context.innerHTML = renderInline(lesson.context);
  ...
  l4Explain.innerHTML = renderInline(lesson.explain);
  l4Question.innerHTML = renderInline(lesson.question);
