...
// ---- page-shell.js concatenation from kernel/page-shell/ ----

// page-shell.js is assembled from single-responsibility sources, in this order.
// `outside: true` means the module is self-contained and is emitted ABOVE the
// shared IIFE (it can then be require()d on its own in a Node test). Everything
...
function pageShellFile() {
  ...
  // A module present on disk but missing from the manifest would be silently
  // dropped from the artefact, so fail loudly instead.
  const known = new Set(["_header.js", ...PAGE_SHELL_MODULES.map((m) => m.file)]);
  const orphans = fs.readdirSync(dir).filter((f) => f.endsWith(".js") && !known.has(f));
  if (orphans.length) {
    throw new Error("kernel/page-shell/: not in PAGE_SHELL_MODULES: " + orphans.join(", "));
  }
...
function main() {
  ...
  // page-shell.js is a root artefact, not a generated/ one. Under --out it still
  // goes at the mirror's top level; audit-gate's drift walker knows which
  // top-level names map back to the repo root rather than to generated/.
  const pageShellDest = outDir ? path.join(outDir, "page-shell.js") : path.join(root, "page-shell.js");
  fs.writeFileSync(pageShellDest, pageShellFile());
  ...
  console.log(`Wrote page-shell.js from ${PAGE_SHELL_MODULES.length} modules in kernel/page-shell/`);
