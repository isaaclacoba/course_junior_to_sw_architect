...
internal sealed class CapstoneSyntax
{
    ...
    public int DistinctInjectedReporterTypes()
        => RunnerCreations()
            ...
            .Select(a => ConcreteReporterType(a.Expression))
            .Where(name => name != null)
            .Distinct()
            .Count();

    // The concrete reporter type behind an injected argument, whether it is
    // created inline (new ConsoleReporter()) or passed through a local variable
    // (IReporter r = new ConsoleReporter(); ... new TestRunner(f, r)). Counting
    // by the type actually instantiated keeps "prove substitutability" honest
    // regardless of how the learner wires the arguments.
    private string? ConcreteReporterType(ExpressionSyntax expr)
    {
        var creation = expr as ObjectCreationExpressionSyntax;
        if (creation == null && expr is IdentifierNameSyntax reference)
        {
            creation = Root.DescendantNodes().OfType<VariableDeclaratorSyntax>()
                .FirstOrDefault(v => v.Identifier.Text == reference.Identifier.Text)
                ?.Initializer?.Value as ObjectCreationExpressionSyntax;
        }

        return creation?.Type is IdentifierNameSyntax id && id.Identifier.Text.Contains("Reporter")
            ? id.Identifier.Text
            : null;
    }
    ...
    // A formatter is, by the task's own definition, a class with a method that
    // takes a bool and returns a string. Detecting by shape (not by the exact
    // words "PASS"/"FAIL") keeps the milestone honest when a learner picks
    // different wording or mistypes the result text.
    private static bool LooksLikeFormatter(ClassDeclarationSyntax cls)
